		...
		Combo = function()
			Iris.Tree({"Combo"})
				Iris.PushConfig({ContentWidth = UDim.new(1, -120)})
					...
					Iris.ComboArray({"Using ComboArray"}, {"Red", "Green", "Blue"}, {index = "No Selection"})
					...
					local InputEnum = Iris.InputEnum({"Using InputEnum"}, Enum.UserInputState, {index = Enum.UserInputState.Begin})
			...
			local styleList = {
				{"Sizing", function()
					...
					for _, vEnum in Enums do
						local Input = Iris.InputEnum({vEnum}, Iris._config[vEnum].EnumType, {index = Iris.WeakState(Iris._config[vEnum])})
		...
		Iris.CollapsingHeader({"Widget Layout"})
			Iris.Tree({"Content Width"})
				...
				Iris.PushConfig({ContentWidth = UDim.new(0, 150)})
					...
					Iris.InputEnum({"axis"}, Enum.Axis, {index = index})
				Iris.PopConfig()
